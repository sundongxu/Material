...
// f[i][j]的语义是：s的前i个字符和p的前j个字符是否匹配。
// 边界条件f[0][0]=true，f[0][j]=true(p的前j个字符都是*）
// 我们可以求解f[i][j]:
// (1) 如果p[j]是字母，则f[i][j]=(p[j-1]==s[i-1]) && f[i-1][j-1]
// (2) 如果p[j]是?，则p[j]匹配任意一个字符，f[i][j]=f[i-1][j-1]
// (3) 如果p[j]是*，分为两种情况：
// a. 如果*代表0个字符的话，则s的前i个字符和p的前j个字符是否匹配，取决于s的前i个字符和p的前j-1个字符是否匹配，即f[i][j]=f[i][j-1]
// b. 如果*代表多个字符的话，则s的前i个字符和p的前j个字符是否匹配，取决于s的前i-1个字符和p的前j个字符是否匹配。
//    如果f[i-1][j]=true，则f[i][j]=true，此时即*代表的字符序列增加了一个字符s[i]而已，而*正好可以可以代表多个不同字符组成的字符串
// 综上a、b两种情况，可得f[i][j]=f[i-1][j] || f[i][j-1]
func isMatch(s string, p string) bool {
